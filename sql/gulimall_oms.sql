...
/*gulimall_oms   gulimall-order  订单管理*/
...
(
    ...
    pay_amount              DECIMAL(18, 4) COMMENT '' 应付金额 '',
    ...
    discount_amount         DECIMAL(18, 4) COMMENT '' 管理员后台调整订单使用的折扣金额 '',
    ...
    auto_confirm_day        INT COMMENT '' 自动确认收货时间（天）'',
...
(
    ...
    promotion_amount   DECIMAL(18, 4) COMMENT '' 商品促销优化金额 '',
    coupon_amount      DECIMAL(18, 4) COMMENT '' 优惠券优惠分解金额 '',
    integration_amount DECIMAL(18, 4) COMMENT '' 积分优惠分解金额 '',
    real_amount        DECIMAL(18, 4) COMMENT '' 商品实际支付金额 '',
    gift_integration   INT COMMENT '' 赠送的积分 '',
    gift_growth        INT COMMENT '' 赠送的成长值 '',
    PRIMARY KEY (id)
) COMMENT ''订单商品信息'';
...
(
    ...
    handle_man      VARCHAR(200) COMMENT '' 处理人 '',
...
(
    ...
    confirm_overtime      INT COMMENT '' 自动确认收货时间（天）'',
    finish_overtime       INT COMMENT '' 自动完成交易时间，不能申请售后（天）'',
    comment_overtime      INT COMMENT '' 自动好评时间（天）'',
    member_level          TINYINT(2) COMMENT '' 会员等级【0->不限会员等级，全部通用；其他-对应等级，可以分别设置】'',
    PRIMARY KEY (id)
) COMMENT ''订单相关设置'';
...
(
    ...
    SUBJECT          VARCHAR(200) COMMENT '' 订单名称 '',
...
(
    id              BIGINT NOT NULL AUTO_INCREMENT COMMENT '' id '',
    order_return_id BIGINT COMMENT '' 退货订单id '',
    refund          DECIMAL(18, 4) COMMENT '' 退款金额 '',
    refund_sn       VARCHAR(64) COMMENT '' 退款申请号 '',
    refund_status   TINYINT(1) COMMENT '' 退款状态 '',
    refund_channel  TINYINT COMMENT '' 退款渠道[1-支付宝；2-微信；3-银联；4-线下退款]'',
