<template>
  <el-dialog
    ...
    <el-form-item label="自动确认收货时间（天）" prop="confirmOvertime">
      <el-input v-model="dataForm.confirmOvertime" placeholder="自动确认收货时间（天）"></el-input>
    </el-form-item>
    <el-form-item label="自动完成交易时间，不能申请售后（天）" prop="finishOvertime">
      <el-input v-model="dataForm.finishOvertime" placeholder="自动完成交易时间，不能申请售后（天）"></el-input>
    </el-form-item>
    <el-form-item label="自动好评时间（天）" prop="commentOvertime">
      <el-input v-model="dataForm.commentOvertime" placeholder="自动好评时间（天）"></el-input>
    </el-form-item>
    <el-form-item label="会员等级【0->不限会员等级，全部通用；其他-对应等级，可以分别设置】" prop="memberLevel">
      <el-input v-model="dataForm.memberLevel" placeholder="会员等级【0->不限会员等级，全部通用；其他-对应等级，可以分别设置】"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          confirmOvertime: [
            { required: true, message: '自动确认收货时间（天）不能为空', trigger: 'blur' }
          ],
          finishOvertime: [
            { required: true, message: '自动完成交易时间，不能申请售后（天）不能为空', trigger: 'blur' }
          ],
          commentOvertime: [
            { required: true, message: '自动好评时间（天）不能为空', trigger: 'blur' }
          ],
          memberLevel: [
            { required: true, message: '会员等级【0->不限会员等级，全部通用；其他-对应等级，可以分别设置】不能为空', trigger: 'blur' }
          ]
        }
      }
    },
